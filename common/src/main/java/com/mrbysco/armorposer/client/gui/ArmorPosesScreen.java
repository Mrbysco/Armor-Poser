...
public class ArmorPosesScreen extends Screen {
	private enum SortType {
		...
		Component getButtonText() {
			return new TranslatableComponent("armorposer.gui.poses.search." + name().toLowerCase(Locale.ROOT));
		}
	}
	...
	public ArmorPosesScreen(ArmorStandScreen parent) {
		super(new TranslatableComponent("armorposer.gui.poses.title"));
	...
	protected void init() {
		...
		this.addRenderableWidget(new Button(centerWidth - (closeButtonWidth / 2) + PADDING, y, closeButtonWidth, 20,
				new TranslatableComponent("gui.cancel"), b -> ArmorPosesScreen.this.onClose()));

		y -= 18 + PADDING;
		this.addRenderableWidget(this.applyButton = new Button(centerWidth - (closeButtonWidth / 2) + PADDING, y,
				closeButtonWidth, 20, new TranslatableComponent("armorposer.gui.poses.selection.apply"), b -> {
			if (selected != null) {
				if (!selected.userAdded() && selected.rawName().equalsIgnoreCase("random")) {
		...
		search = new EditBox(getScreenFont(), centerWidth - listWidth / 2 + PADDING + 1, y, listWidth - 2, 14,
				new TranslatableComponent("armorposer.gui.poses.search"));

		int fullButtonHeight = PADDING + 20 + PADDING;
		this.poseListWidget[0] = new PoseListWidget(this, new TranslatableComponent("armorposer.gui.poses.default"),
				false, listWidth, fullButtonHeight, search.y - getScreenFont().lineHeight - PADDING);
		this.poseListWidget[0].setLeftPos(0);
		this.poseListWidget[0].setY(10);
		this.poseListWidget[0].setHeight(this.height);

		this.poseListWidget[1] = new PoseListWidget(this, new TranslatableComponent("armorposer.gui.poses.user"),
				true, listWidth, fullButtonHeight, search.y - getScreenFont().lineHeight - PADDING);
		this.poseListWidget[1].setLeftPos(width - listWidth);
		...
		addRenderableWidget(SortType.A_TO_Z.button = new Button(x, PADDING, width - buttonMargin, 20,
				SortType.A_TO_Z.getButtonText(), b -> resortPoses(SortType.A_TO_Z)));
		x += width + buttonMargin;
		addRenderableWidget(SortType.Z_TO_A.button = new Button(x, PADDING, width - buttonMargin, 20,
				SortType.Z_TO_A.getButtonText(), b -> resortPoses(SortType.Z_TO_A)));
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
		this.poseListWidget[0].render(poseStack, mouseX, mouseY, partialTicks);
		this.poseListWidget[1].render(poseStack, mouseX, mouseY, partialTicks);
		super.render(poseStack, mouseX, mouseY, partialTicks);

		Component text = new TranslatableComponent("armorposer.gui.poses.search");
		drawCenteredString(poseStack, getScreenFont(), text, this.width / 2 + PADDING,
				search.y - getScreenFont().lineHeight - 2, 0xFFFFFF);
